...
/// Copyright 2017-2021 by Trevor SANDY.
///
    ...
    class WinConOptionsProcessor
    {
      public:
        ...
        struct WinPath
        {
            ...
            WinPath(const std::string &s, bool desc = false, bool wrt = false) : str(s), descend(desc), writable(wrt) { }
        };
     ...
     protected:
        ...
        std::string win_getcwd(void);
        std::string basename(const std::string &path);
        std::string dirname(const std::string &path);
        std::string win_readlink(const std::string &path);
        std::string pre_process_conf_line(const std::string &input);
        void add_permitted_path(std::list<WinPath> &paths, const std::string &input, const std::string &conf_name, unsigned long line_number);
        ...
        ShellOut m_shellout;
        std::list<WinPath> m_permitted_paths;
